...
export default function AudioTimeline({addKeyframe, goToKeyframe, activeMicCharacterId, onPlayChange}) {
  const [sound, setSound] = useState<Sound | null>(null);
//   export const soundRef = useRef<Sound | null>(null);
   ...
   if (selectedChunkId) {
    setChunks((prevChunks) =>
        ...
        chunk.id === selectedChunkId
            ? { ...chunk, user: activeMicCharacterId } // ✅ immutably update user
            : chunk // ✅ return unchanged chunk
        )
    );
    }
...
useEffect(() => {
  ...
  // only update keyframe when audio is playing
  if (!isPlaying) return;
 ...
 const loadAudio = () => {
  // Example: audio file placed in `android/app/src/main/res/raw/your-audio-file.mp3`
  // or added to iOS project in Xcode (bundle resource)
  ...
  const audioFile = new Sound('aa.mp3', Sound.MAIN_BUNDLE, (error) => {
    ...
    // Store duration + sound object
    setDuration(audioDuration);
    setSound(audioFile);

    // Create one full-length chunk initially
    setChunks([{
      id: '1',
      startTime: 0,
  ...
  const togglePlayback = () => {
    if (!sound) {
      ...
      onPlayChange(!isPlaying); // ✅ tell App
      return;
    }
