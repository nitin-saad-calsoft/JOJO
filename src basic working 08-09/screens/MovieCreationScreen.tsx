...
//   const handleChunkActive = (characterId: string | null) => {
//   if (!characterId) {
//     // reset all to idle when no chunk active
//     setCharacters((prev) =>
//       prev.map((char) => ({ ...char, animation: 'idle' }))
//     );
//     return;
//   }
...
console.log("------key frames", keyframes, characters)
  return (
        ...
        <View style={styles.canvasBackground}>
          ...
          {characters.map((character) => {
            ...
            return (
              ...
              >
                <View style={{ position: 'absolute', top: -30, left: '50%', transform: [{ translateX: -12 }] }}>
                  <TouchableOpacity
                    onPress={() => {
                      setMicStates(prev => {
                        ...
                        // current character ko toggle karo
                        updated[character.id] = !prev[character.id];
                        setActiveMicCharacterId(character.id)
                        setKeyframes(prev => {
                      const allFrames = [...prev];

                      allFrames[currentKeyframeIndex] = {
                        ...allFrames[currentKeyframeIndex],
                      ...
                      return allFrames;
                        ...
                        // agar mic ON hai to keyframe update karo
                        // if (updated[character.id]) {
                        //   updateCurrentKeyframe(
                        //     characters.map(c =>
                        //       c.id === character.id
                        //         ? { ...c, micOn: true } // 👈 micOn flag add kar lo
                        //         : { ...c, micOn: false }
                        //     )
                        //   );
                        // } else {
                        //   // agar mic OFF kar diya to keyframe bhi update
                        //   updateCurrentKeyframe(
                        //     characters.map(c => ({ ...c, micOn: false }))
                        //   );
                        // }
                    ...
                    style={{
                      ...
                      height: 24,
                      borderRadius: 12,
                      backgroundColor: keyframes[currentKeyframeIndex]?.assignCharacter === character.id ? 'green' : 'gray',
                      alignItems: 'center',
