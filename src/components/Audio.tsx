...
export default function AudioTimeline({
  ...
  // --- EFFECT: Analyze audio once ---
  useEffect(() => {
    analyzeAudio();
  }, [analyzeAudio]);

  // --- EFFECT: Scroll timeline to current position ---
  useEffect(() => {
    if (scrollViewRef.current) {
      const currentOffset = (position / 1000) * PIXELS_PER_SECOND;
      scrollViewRef.current.scrollTo({ x: currentOffset, y: 0, animated: false });
    }
  }, [position, isPlaying]);

  // --- EFFECT: Notify parent of time update ---
  useEffect(() => {
    onTimeUpdate?.(position);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [position]);

  // --- EFFECT: Handle chunk activity and keyframe jump ---
  useEffect(() => {
    if (!duration || !isPlaying || chunks.length === 0) return;
    ...
    if (activeIndex !== -1) {
      ...
      if (activeChunk?.characterId) onChunkActive?.(activeChunk.characterId);
    } else {
      onChunkActive?.(null);
    }
    if (!isPlaying) onChunkActive?.(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [position, duration, isPlaying, chunks]);

  // --- EFFECT: Handle assign/unassign chunk to character ---
  useEffect(() => {
    if (unassignTrigger?.characterId) {
      setChunks(prev =>
        prev.map(chunk =>
  ...
  useEffect(() => {
    if (assignChunkToCharacter?.chunkId) {
      setChunks(prev =>
        prev.map(chunk =>
  ...
  // --- EFFECT: Notify parent of selected chunk's characterId ---
  useEffect(() => {
  ...
  // --- EFFECT: Assign chunk to activeMicCharacterId only if needed ---
  useEffect(() => {
  ...
  // --- EFFECT: Auto-select chunk under red line and jump to keyframe ---
  useEffect(() => {
    if (!duration || chunks.length === 0) return;
    const redLineTimeMs = (scrollPosition / PIXELS_PER_SECOND) * 1000;
    ...
    if (currentChunk && currentChunk.id !== selectedChunkId) {
      ...
      if (index !== -1) goToKeyframe(index);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scrollPosition, duration, chunks]);
