      ...
      setChunks((prevChunks) =>
        prevChunks.map((chunk) =>
          chunk.id === selectedChunkId
            ? { ...chunk, user: activeMicCharacterId, characterId: activeMicCharacterId }
            : chunk
        )
      );
    }
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
          onPressIn={() => {
            // Prevent scroll interference when selecting chunk
          }}
        >
            {chunk.characterId && (
            <View style={styles.chunkCharacterLabel}>
              <Image
                source={{ uri: characters.find((c:any) => c.name === chunk.characterId)?.preview }}
